...
ECHO 3: Service ID  : %3
ECHO.
ECHO Creating AW 2008 OLAP databases.
...
sqlcmd -S %_instancename% -E -Q "USE AdventureWorksDW2008; EXEC sp_grantdbaccess N'%_serviceid%'; EXEC sp_addrolemember 'db_datareader', N'%_serviceid%';"

ECHO Deploy and process the enterprise version of the cube.
"%_sourcepath%ascmd.exe" -S "%_instancename%" -i "%_sourcepath%AdventureWorks 2008 Analysis Services Project\enterprise\Adventure Works DW 2008 Script.xmla"

ECHO Deploy and process the standard version of the cube.
"%_sourcepath%ascmd.exe" -S "%_instancename%" -i "%_sourcepath%AdventureWorks 2008 Analysis Services Project\standard\Adventure Works DW 2008 SE Script.xmla"
